...
(set-info :ultimate-id benchmark43_conjunctive_abstracted.i_AllErrorsAtOnce_Iteration1)
...
(declare-fun |main_#t~bitwise3_6| () Int)
...
(assert (! (and (or (= (ite (< main_~x~0_4 100) 1 0) 0) (= (ite (< main_~y~0_4 100) 1 0) 0)) (= |main_#t~bitwise3_6| 0)) :named ssa_6))
(assert (! (= |main_#t~bitwise3_6| 0) :named ssa_7))
(assert (! true :named ssa_8_GlobVarAssigCall))
(assert (! (= |__VERIFIER_assert_#in~cond_8| (ite (or (= main_~y~0_4 100) (= main_~x~0_4 100)) 1 0)) :named ssa_8_LocVarAssigCall))
